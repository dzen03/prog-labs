...
import com.dzen03.lab8.DrawActivity;
import com.dzen03.lab8.ShowActivity;
 ...
 */
public class CustomPanZoom implements View.OnTouchListener {
    ...
    protected CustomPanZoom(@NonNull XYPlot plot, Pan pan, Zoom zoom) {
        this.plot = plot;
        this.pan = pan;
        this.zoom = zoom;
    ...
    protected CustomPanZoom(@NonNull XYPlot plot, Pan pan, Zoom zoom, ZoomLimit limit) {
        this.plot = plot;
        this.pan = pan;
        this.zoom = zoom;
     ...
     * Use {@link CustomPanZoom#attach(XYPlot, Pan, Zoom, ZoomLimit)} for finer grain control of this behavior.
     * @param plot
     * @return
     */
    public static CustomPanZoom attach(@NonNull XYPlot plot) {
     ...
     * @return
     */
    public static CustomPanZoom attach(@NonNull XYPlot plot, @NonNull Pan pan, @NonNull Zoom zoom) {
     ...
     * @return
     */
    public static CustomPanZoom attach(@NonNull XYPlot plot, @NonNull Pan pan, @NonNull Zoom zoom, @NonNull ZoomLimit limit) {
        CustomPanZoom pz = new CustomPanZoom(plot, pan, zoom, limit);
    ...
    protected void click(final MotionEvent motionEvent) {
        ...
        if (oldFirstFinger.equals(firstFingerPos))
        {
            Number x = plot.getXVal(firstFingerPos);
            Number y = plot.getYVal(firstFingerPos);

            if (x == null || state.domainLowerBoundary == null)
                return;
            DrawActivity.AsyncPick pick = new DrawActivity.AsyncPick();
            pick.execute(x.doubleValue(), y.doubleValue(), Math.abs(state.domainUpperBoundary.doubleValue() - state.domainLowerBoundary.doubleValue()),
                    Math.abs(state.rangeUpperBoundary.doubleValue() - state.rangeLowerBoundary.doubleValue()));
    ...
    public void reset(final MotionEvent event)
    {
        click(event);
        reset();
    }
