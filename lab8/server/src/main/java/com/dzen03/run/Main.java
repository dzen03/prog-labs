...
//                    final SocketAddress[] address = {null};
                    ConcurrentHashMap<SocketAddress, Transmission> packets = new ConcurrentHashMap<>();

//                    final ArrayList<Integer> packetCount = Collections.synchronizedList(new ArrayList<>());
                    final Vector<Integer> packetCount = new Vector<>();
                    packetCount.add(Integer.MAX_VALUE);


                    final boolean[] reading = {true};
                    for (final int[] i = {0}; i[0] < packetCount.get(0); )
                    {
//                        LOGGER.log(Level.INFO, String.format("---i=%d, cnt=%d", i[0], packetCount.get(0)));
                        int finalI = i[0];
...
//                                LOGGER.log(Level.INFO, String.format("addr=%s", address.toString()));
                                ++i[0];
                                buf.flip();
                                ...
                                {
                                    packets.get(address).count[0] = (int) tmp[1];
                                    packets.get(address).bytes.put(ind, dt);

                                    synchronized (packetCount)
                                    {
                                        if (packetCount.get(0) == Integer.MAX_VALUE)
                                        {
                                            packetCount.set(0, (int) tmp[1]);
                                        }
                                        else
                                        {
                                            packetCount.set(0, (int) tmp[1]);
                                ...
                                else
                                {
                                    synchronized (packetCount)
                                    {
                                        ...
                                        {
                                            packetCount.set(0, packetCount.get(0) + (int) tmp[1]);
                                        }
                                    }
                                    packets.put(address, new Transmission(ind, dt));
                                    packets.get(address).count[0] = (int) tmp[1];
                                }
...
//                                LOGGER.log(Level.INFO, String.format("i=%d, cnt=%d", i[0], packetCount.get(0)));
                    ...
                    while (!(packets.isEmpty() && executed))
                    for (var address : packets.keySet())
                    {
                        final ConcurrentHashMap<Integer, ArrayList<Byte>> bytes = (ConcurrentHashMap<Integer, ArrayList<Byte>>) packets.get(address).bytes;
                        int[] count = packets.get(address).count;
//                        LOGGER.log(Level.INFO, String.valueOf(bytes.size()) + count[0]);
                        if (bytes.size() != count[0])
                            continue;
                        executed = true;
//                        packets.get(address).bytes.clear();
//                        packets.get(address).count[0] = -1;
...
//                        LOGGER.log(Level.INFO, Arrays.toString(result));
                        ...
                        try
                        {
                            ...
                            ExecuteProcessor executeProcessor = new ExecuteProcessor(commandClass.type, arg, readChannel, address);
                               ...
                               AuthException ex)
                        {
                            ...
                            pool.execute(new SendProcessor(data, 0, data.length, readChannel, address));
                            continue;
                        }
                        finally
                        {
